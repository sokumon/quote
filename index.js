...
async function preload_things() {
    const promises = quotes.map(async function(element) {
        element.text = element.text.replace(/[^a-zA-Z ]/g, "").toLowerCase();
        let response = await fetch(`./models/${element.text}.glb`);
        let blob = await response.blob();
        let content = await readBlob(blob);
        all_models[element.text] = content;
    });
...
async function start_audio() {
    try {
        // Preload all models before starting audio
        await preload_things();
        console.log('All models preloaded:', Object.keys(all_models).length);
        ...
        function syncModels() {
            ...
            for (let i = 0; i < quotes.length; i++) {
                let quote = quotes[i];
                quote.text = quote.text.replace(/[^a-zA-Z ]/g, "").toLowerCase();
                if (currentTime >= (quote.start / 1000) && currentTime <= (quote.end / 1000)) {
                    if (model_viewver.src !== all_models[quote.text]) {
                        model_viewver.src = all_models[quote.text];
                    }
                }
            }
            requestAnimationFrame(syncModels);
        }
